...
/* tu5xx: a5xx 着色器状态，移植自 fd5 (fd5_emit_shader_obj / fd5_emit_shader /
 * fd5_program_emit 的 SP_xS_CTRL_REG0 部分)。仅支持 VS/FS（tess/geom 待补）。 */
static void
tu5_emit_xs(struct tu_cs *cs,
            gl_shader_stage stage,
            ...
            uint64_t binary_iova)
{
   ...
   case MESA_SHADER_FRAGMENT:
      break;
   default:
      unreachable("tu5xx: unsupported shader stage");
   }

   const bool is_vs = stage == MESA_SHADER_VERTEX;
   enum a3xx_threadsize fssz =
      xs->info.double_threadsize ? FOUR_QUADS : TWO_QUADS;
   const uint32_t instrlen = xs->instrlen;

   /* SP_xS_CTRL_REG0（fd5_program_emit） */
   if (is_vs) {
      tu_cs_emit_pkt4(cs, REG_A5XX_SP_VS_CTRL_REG0, 1);
      tu_cs_emit(cs,
                 A5XX_SP_VS_CTRL_REG0_HALFREGFOOTPRINT(xs->info.max_half_reg + 1) |
                    A5XX_SP_VS_CTRL_REG0_FULLREGFOOTPRINT(xs->info.max_reg + 1) |
                    COND(instrlen != 0, A5XX_SP_VS_CTRL_REG0_BUFFER) |
                    /* XXX: blob 常置位 */
                    0x2 |
                    A5XX_SP_VS_CTRL_REG0_BRANCHSTACK(ir3_shader_branchstack_hw(xs)) |
                    COND(xs->need_pixlod, A5XX_SP_VS_CTRL_REG0_PIXLODENABLE));
   } else {
      tu_cs_emit_pkt4(cs, REG_A5XX_SP_FS_CTRL_REG0, 1);
      tu_cs_emit(cs,
                 COND(xs->total_in > 0, A5XX_SP_FS_CTRL_REG0_VARYING) |
                    /* XXX: blob 几乎处处置位 */
                    0x40002 |
                    COND(instrlen != 0, A5XX_SP_FS_CTRL_REG0_BUFFER) |
                    A5XX_SP_FS_CTRL_REG0_THREADSIZE(fssz) |
                    A5XX_SP_FS_CTRL_REG0_HALFREGFOOTPRINT(xs->info.max_half_reg + 1) |
                    A5XX_SP_FS_CTRL_REG0_FULLREGFOOTPRINT(xs->info.max_reg + 1) |
                    A5XX_SP_FS_CTRL_REG0_BRANCHSTACK(ir3_shader_branchstack_hw(xs)) |
                    COND(xs->need_pixlod, A5XX_SP_FS_CTRL_REG0_PIXLODENABLE));
   }

   /* SP_xS_OBJ_START + pvtmem（fd5_emit_shader_obj） */
   const uint16_t reg_obj_start =
      is_vs ? REG_A5XX_SP_VS_OBJ_START_LO : REG_A5XX_SP_FS_OBJ_START_LO;
   tu_cs_emit_pkt4(cs, reg_obj_start, 6);
   tu_cs_emit_qw(cs, binary_iova);
   tu_cs_emit(cs,
              A5XX_SP_VS_PVT_MEM_PARAM_MEMSIZEPERITEM(pvtmem->per_fiber_size) |
                 A5XX_SP_VS_PVT_MEM_PARAM_HWSTACKOFFSET(pvtmem->per_sp_size));
   if (pvtmem->iova) {
   ...
   tu_cs_emit(cs,
              A5XX_SP_VS_PVT_MEM_SIZE_TOTALPVTMEMSIZE(pvtmem->per_sp_size));
   /* tu5xx: a5xx PVT_MEM_SIZE 无 PERWAVEMEMLAYOUT 字段（a6xx 起才有） */

   /* 着色器二进制：CP_LOAD_STATE4(ST4_SHADER, INDIRECT)（fd5_emit_shader），
    * instrlen == 0 表示从外部内存执行，不做内部 buffer 加载 */
   if (instrlen) {
      tu_cs_emit_pkt7(cs, CP_LOAD_STATE4, 3);
      tu_cs_emit(cs, CP_LOAD_STATE4_0_DST_OFF(0) |
                        CP_LOAD_STATE4_0_STATE_SRC(SS4_INDIRECT) |
                        CP_LOAD_STATE4_0_STATE_BLOCK(is_vs ? SB4_VS_SHADER
                                                           : SB4_FS_SHADER) |
                        CP_LOAD_STATE4_0_NUM_UNIT(instrlen));
      tu_cs_emit(cs, (uint32_t) binary_iova |
                        CP_LOAD_STATE4_1_STATE_TYPE(ST4_SHADER));
      tu_cs_emit(cs, (uint32_t) (binary_iova >> 32));
   }

   /* immediates（fd5_emit_const_user 布局） */
   const struct ir3_const_state *const_state = ir3_const_state(xs);
   uint32_t base = const_state->offsets.immediate;
   unsigned immediate_size = tu_xs_get_immediates_packet_size_dwords(xs);

   if (immediate_size > 0) {
      tu_cs_emit_pkt7(cs, CP_LOAD_STATE4, 3 + immediate_size);
      tu_cs_emit(cs, CP_LOAD_STATE4_0_DST_OFF(base) |
                        CP_LOAD_STATE4_0_STATE_SRC(SS4_DIRECT) |
                        CP_LOAD_STATE4_0_STATE_BLOCK(is_vs ? SB4_VS_SHADER
                                                           : SB4_FS_SHADER) |
                        CP_LOAD_STATE4_0_NUM_UNIT(immediate_size / 4));
   ...
   if (const_state->constant_data_ubo != -1) {
      ...
      /* UBO state（fd5_image.c ST4_UBO 布局：每项 2 dwords 地址） */
      tu_cs_emit_pkt7(cs, CP_LOAD_STATE4, 3 + 2);
      tu_cs_emit(cs,
                 CP_LOAD_STATE4_0_DST_OFF(const_state->constant_data_ubo) |
                    CP_LOAD_STATE4_0_STATE_SRC(SS4_DIRECT) |
                    CP_LOAD_STATE4_0_STATE_BLOCK(is_vs ? SB4_VS_SHADER
                                                       : SB4_FS_SHADER) |
                    CP_LOAD_STATE4_0_NUM_UNIT(1));
      tu_cs_emit(cs, CP_LOAD_STATE4_1_EXT_SRC_ADDR(0) |
                        CP_LOAD_STATE4_1_STATE_TYPE(ST4_UBO));
      tu_cs_emit(cs, CP_LOAD_STATE4_2_EXT_SRC_ADDR_HI(0));
      tu_cs_emit(cs, (uint32_t) iova);
      tu_cs_emit(cs, (uint32_t) (iova >> 32));

      /* constant data 拷入 const file（fd5_emit_const_bo 布局） */
      ...
      for (int i = 0; i < ubo_state->num_enabled; i++) {
         ...
         tu_cs_emit_pkt7(cs, CP_LOAD_STATE4, 3);
         tu_cs_emit(cs,
                    CP_LOAD_STATE4_0_DST_OFF(ubo_state->range[i].offset / 16) |
                       CP_LOAD_STATE4_0_STATE_SRC(SS4_INDIRECT) |
                       CP_LOAD_STATE4_0_STATE_BLOCK(is_vs ? SB4_VS_SHADER
                                                          : SB4_FS_SHADER) |
                       CP_LOAD_STATE4_0_NUM_UNIT(size / 16));
         tu_cs_emit(cs, (uint32_t) (iova + start) |
                           CP_LOAD_STATE4_1_STATE_TYPE(ST4_CONSTANTS));
         tu_cs_emit(cs, (uint32_t) ((iova + start) >> 32));
      }
   }

   /* 静态 FS driver param（fd5_emit_const_user 布局） */
   if (!is_vs && const_state->num_driver_params > 0) {
      ...
      if (size > 0) {
         ...
         tu_cs_emit(cs, CP_LOAD_STATE4_0_DST_OFF(base) |
                           CP_LOAD_STATE4_0_STATE_SRC(SS4_DIRECT) |
                           CP_LOAD_STATE4_0_STATE_BLOCK(SB4_FS_SHADER) |
                           CP_LOAD_STATE4_0_NUM_UNIT(size));
         tu_cs_emit(cs, CP_LOAD_STATE4_1_EXT_SRC_ADDR(0) |
                           CP_LOAD_STATE4_1_STATE_TYPE(ST4_CONSTANTS));
         tu_cs_emit(cs, CP_LOAD_STATE4_2_EXT_SRC_ADDR_HI(0));
            ...
            uint64_t binary_iova)
{
   if (cs->device->physical_device->info->chip == 5) {
      tu5_emit_xs(cs, stage, xs, pvtmem, binary_iova);
      return;
   }
                  ...
                  const struct ir3_shader_variant *vs)
{
   ...
   if (CHIP == A5XX) {
      /* tu5xx: a5xx VFD_CONTROL_0 只有 VTXCNT（fd5_emit_vertex_bufs） */
      tu_cs_emit_pkt4(cs, REG_A5XX_VFD_CONTROL_0, 1);
      tu_cs_emit(cs, A5XX_VFD_CONTROL_0_VTXCNT(attr_count));
   } else {
      tu_cs_emit_regs(cs,
                      A6XX_VFD_CONTROL_0(
   ...
   if (attr_count) {
      tu_cs_emit_pkt4(cs,
                      CHIP == A5XX ? REG_A5XX_VFD_DEST_CNTL_INSTR(0)
                                   : REG_A6XX_VFD_DEST_CNTL_INSTR(0),
                      attr_count);
   }
   ...
   for (unsigned i = 0; i < attr_count; i++) {
      if (input_for_attr[i] >= 0) {
            ...
            /* tu5xx: VFD_DEST_CNTL 位排布与 a6xx 相同（WRITEMASK/REGID） */
...
tu6_emit_vs(struct tu_cs *cs,
            ...
            uint32_t view_mask)
{
   /* tu5xx: fd5 无 multiview；VFD_CONTROL_1..5 由 VS 状态组发射 */
   if (CHIP == A5XX) {
      ...
      /* VFD_CONTROL_1..5 连续寄存器（fd5_program_emit） */
      tu_cs_emit_pkt4(cs, REG_A5XX_VFD_CONTROL_1, 5);
      tu_cs_emit(cs, A5XX_VFD_CONTROL_1_REGID4VTX(vertexid_regid) |
                     A5XX_VFD_CONTROL_1_REGID4INST(instanceid_regid) |
                     0xfc0000);
