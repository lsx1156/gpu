                        ...
                        struct tu_instance *instance)
{
   ...
   case 5: {
      /* tu5xx: a5xx (Adreno 506+) 尚无 CCU 语义，按保守值初始化。
       * 目标是先打通物理设备枚举，其余逐层补齐。 */
      device->dev_info = info;
      device->info = &device->dev_info;
      device->ccu_offset_bypass = 0;
      device->ccu_offset_gmem = 0;
      device->usable_sets = 0;
                ...
                VkDevice *pDevice)
{
   ...
   case 5:
      vk_device_dispatch_table_from_entrypoints(
         &dispatch_table, &tu_device_entrypoints_a5xx, false);
      break;
   case 6:
      vk_device_dispatch_table_from_entrypoints(
