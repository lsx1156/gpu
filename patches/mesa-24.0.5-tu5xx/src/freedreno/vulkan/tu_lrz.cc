...
static bool
tu_lrz_disabled(struct tu_device *dev)
{
   /* tu5xx: a5xx LRZ 暂不支持（寄存器/事件与 a6xx 差异大），彻底禁用，
    * 避免 a6xx 寄存器泄漏进命令流造成 protected mode error。 */
   return dev->physical_device->info->chip == 5;
}

static void
tu6_emit_lrz_buffer(struct tu_device *dev, struct tu_cs *cs,
                    struct tu_image *depth_image)
{
   if (tu_lrz_disabled(dev))
      return;
...
tu_lrz_begin_renderpass(struct tu_cmd_buffer *cmd)
{
   ...
   if (!cmd->state.lrz.valid) {
      tu6_emit_lrz_buffer(cmd->device, &cmd->cs, NULL);
   }
}
...
tu_lrz_tiling_begin(struct tu_cmd_buffer *cmd, struct tu_cs *cs)
{
   ...
   tu6_emit_lrz_buffer(cmd->device, cs, lrz->image_view->image);
...
tu_lrz_tiling_end(struct tu_cmd_buffer *cmd, struct tu_cs *cs)
{
   if (cmd->state.lrz.fast_clear || cmd->state.lrz.gpu_dir_tracking) {
      tu6_emit_lrz_buffer(cmd->device, cs, cmd->state.lrz.image_view->image);
               ...
               struct tu_image *image)
{
   ...
   tu6_emit_lrz_buffer(cmd->device, cs, image);
   tu6_disable_lrz_via_depth_view(cmd, cs);
}
                         ...
                         const VkImageSubresourceRange *pRanges)
{
   ...
   tu6_emit_lrz_buffer(cmd->device, &cmd->cs, image);
