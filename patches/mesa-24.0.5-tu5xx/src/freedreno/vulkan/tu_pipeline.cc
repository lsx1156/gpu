...
{
   if (CHIP == A5XX) {
      /* tu5xx: a5xx 的 SP/HLSQ 每级配置（含 constoff/instroff 分配，fd5
       * 跨 stage 累积）由 tu6_emit_program_config<A5XX> 整体发射。 */
      return;
   }
...
{
   /* tu5xx: CP_LOAD_STATE6 为 a6xx 专属指令；a5xx 的动态偏移待
    * CP_LOAD_STATE4 路径补齐，首个三角形无描述符集，直接跳过。 */
...
tu6_emit_shared_consts_enable(struct tu_cs *cs, bool enable)
{
   ...
   /* tu5xx: a5xx 无 SP_MODE_CONTROL，ISAM 相关由 SP_SP_CNTL（program config）负责 */
   if (CHIP == A5XX)
      return;
...
/* tu5xx: a5xx VPC 连线（移植自 fd5_program_emit 的 VPC 段）。仅支持
 * VS→FS（无 tess/GS）。a5xx 将 pos/psize 追加到 linkage 末尾，且需要
 * VPC_VAR_DISABLE 位图（varmask 在追加 pos/psize 前发射）。 */
static uint32_t
tu5_emit_vpc(struct tu_cs *cs,
             ...
             const struct ir3_shader_variant *fs)
{
   ...
   if (fs)
      ir3_link_shaders(&linkage, vs, fs, true);

   const uint32_t pos_regid =
      ir3_find_output_regid(vs, VARYING_SLOT_POS);
   const uint32_t psize_regid =
      ir3_find_output_regid(vs, VARYING_SLOT_PSIZ);
   const uint32_t clip0_regid =
      ir3_find_output_regid(vs, VARYING_SLOT_CLIP_DIST0);
   const uint32_t clip1_regid =
      ir3_find_output_regid(vs, VARYING_SLOT_CLIP_DIST1);

   /* VPC_VAR_DISABLE 需在追加 pos/psize 之前发射（fd5 模式） */
   tu_cs_emit_pkt4(cs, REG_A5XX_VPC_VAR_DISABLE(0), 4);
   for (unsigned i = 0; i < 4; i++)
      tu_cs_emit(cs, ~linkage.varmask[i]);

   uint8_t clip0_loc = linkage.clip0_loc, clip1_loc = linkage.clip1_loc;

   /* a5xx appends pos/psize to end of the linkage map: */
   if (VALIDREG(pos_regid))
      ir3_link_add(&linkage, VARYING_SLOT_POS, pos_regid, 0xf, linkage.max_loc);

   uint32_t psize_loc = 0xff;
   if (VALIDREG(psize_regid)) {
      psize_loc = linkage.max_loc;
      ir3_link_add(&linkage, VARYING_SLOT_PSIZ, psize_regid, 0x1, linkage.max_loc);
   }

   /* Handle the case where clip/cull distances aren't read by the FS */
   uint8_t clip_cull_mask = vs->clip_mask | vs->cull_mask;
   if (clip0_loc == 0xff && VALIDREG(clip0_regid) && (clip_cull_mask & 0xf)) {
      clip0_loc = linkage.max_loc;
      ir3_link_add(&linkage, VARYING_SLOT_CLIP_DIST0, clip0_regid,
                   clip_cull_mask & 0xf, linkage.max_loc);
   }
   if (clip1_loc == 0xff && VALIDREG(clip1_regid) && (clip_cull_mask >> 4)) {
      clip1_loc = linkage.max_loc;
      ir3_link_add(&linkage, VARYING_SLOT_CLIP_DIST1, clip1_regid,
                   clip_cull_mask >> 4, linkage.max_loc);
   }

   /* The GPU hangs when there are no outputs, add a dummy output */
   if (linkage.cnt == 0)
   ...
   /* tu5xx: SP_VS_OUT/VPC_DST 位排布与 a6xx 相同，寄存器地址不同 */
   tu_cs_emit_pkt4(cs, REG_A5XX_SP_VS_OUT(0), sp_out_count);
   tu_cs_emit_array(cs, sp_out, sp_out_count);

   tu_cs_emit_pkt4(cs, REG_A5XX_SP_VS_VPC_DST(0), sp_vpc_dst_count);
   tu_cs_emit_array(cs, sp_vpc_dst, sp_vpc_dst_count);

   tu_cs_emit_pkt4(cs, REG_A5XX_PC_PRIM_VTX_CNTL, 1);
   tu_cs_emit(cs, COND(vs->writes_psize, A5XX_PC_PRIM_VTX_CNTL_PSIZE));

   tu_cs_emit_pkt4(cs, REG_A5XX_SP_PRIMITIVE_CNTL, 1);
   tu_cs_emit(cs, A5XX_SP_PRIMITIVE_CNTL_VSOUT(linkage.cnt));

   tu_cs_emit_pkt4(cs, REG_A5XX_VPC_CNTL_0, 1);
   tu_cs_emit(cs, A5XX_VPC_CNTL_0_STRIDE_IN_VPC(linkage.max_loc) |
                  COND(fs && fs->total_in > 0, A5XX_VPC_CNTL_0_VARYING) |
                  0x10000); /* XXX fd5 */

   tu_cs_emit_pkt4(cs, REG_A5XX_VPC_PACK, 1);
   ...
   if (fs) {
      ...
      for (int i = -1;
           (i = ir3_next_varying(fs, i)) < (int) fs->inputs_count;) {
         if (!(fs->inputs[i].flat || fs->inputs[i].rasterflat))
            continue;

         uint32_t loc = fs->inputs[i].inloc;
         for (unsigned c = 0; c < 4; c++) {
            if (fs->inputs[i].compmask & (1 << c)) {
               vinterp[loc / 16] |= 1 << ((loc % 16) * 2); /* 1 = FLAT */
               loc++;
            }
         }
      }
   ...
   tu_cs_emit_pkt4(cs, REG_A5XX_VPC_VARYING_PS_REPL_MODE(0), 8);
   tu_cs_emit_array(cs, (uint32_t[8]) {0}, 8);

   tu_cs_emit_pkt4(cs, REG_A5XX_GRAS_VS_CL_CNTL, 1);
   tu_cs_emit(cs, A5XX_GRAS_VS_CL_CNTL_CLIP_MASK(vs->clip_mask) |
                  A5XX_GRAS_VS_CL_CNTL_CULL_MASK(vs->cull_mask));

   tu_cs_emit_pkt4(cs, REG_A5XX_VPC_CLIP_CNTL, 1);
   tu_cs_emit(cs, A5XX_VPC_CLIP_CNTL_CLIP_MASK(clip_cull_mask) |
                  A5XX_VPC_CLIP_CNTL_CLIP_DIST_03_LOC(clip0_loc) |
                  A5XX_VPC_CLIP_CNTL_CLIP_DIST_47_LOC(clip1_loc));

   tu_cs_emit_pkt4(cs, REG_A5XX_PC_CLIP_CNTL, 1);
   tu_cs_emit(cs, A5XX_PC_CLIP_CNTL_CLIP_MASK(vs->clip_mask));

   return linkage.max_loc;
}
...
/* tu5xx: a5xx 程序配置，移植自 fd5_program_emit（fd5_program.c）。
 * 包含 SP/HLSQ 每级 CONFIG/CNTL/CONSTLEN/INSTRLEN（constoff/instroff 按
 * fd5 setup_stages 分配），以及 FS sysval 相关的 HLSQ_CONTROL_0..4、
 * GRAS_CNTL、RB_RENDER_CONTROL0/1、SP_FS_OUTPUT_REG 等。 */
static void
tu5_emit_program_config(struct tu_cs *cs,
                        ...
                        const struct ir3_shader_variant **variants)
{
   ...
   tu6_emit_shared_consts_enable<A5XX>(cs, shared_consts_enable);

   /* fd5 setup_stages(): constoff 按 VS,FS,HS,DS,GS 累积（单位: 4*vec4），
    * instroff VS=0，其余 = 64 - FS.instrlen（VS+FS > 64 时外部内存）。 */
   const gl_shader_stage a5xx_stage_order[] = {
      MESA_SHADER_VERTEX, MESA_SHADER_FRAGMENT,
      MESA_SHADER_TESS_CTRL, MESA_SHADER_TESS_EVAL, MESA_SHADER_GEOMETRY,
   };
   struct {
      const struct ir3_shader_variant *v;
      uint8_t constoff, constlen, instrlen, instroff;
   } st[5] = {};

   uint8_t running_constoff = 0;
   for (unsigned i = 0; i < ARRAY_SIZE(a5xx_stage_order); i++) {
      const struct ir3_shader_variant *v = variants[a5xx_stage_order[i]];
      if (!v)
         continue;
      assert(v->constlen % 4 == 0);
      st[i].v = v;
      st[i].constlen = v->constlen / 4;
      st[i].instrlen = v->instrlen;
      st[i].constoff = running_constoff;
      running_constoff += st[i].constlen;
   }

   if (st[0].instrlen + st[1].instrlen > 64) {
      if (st[1].instrlen < 64) {
         st[0].instrlen = 0;
      } else if (st[0].instrlen < 64) {
         st[1].instrlen = 0;
      } else {
         st[0].instrlen = 0;
         st[1].instrlen = 0;
   ...
   st[0].instroff = 0;
   st[1].instroff = 64 - st[1].instrlen;
   st[2].instroff = st[3].instroff = st[4].instroff = st[1].instroff;

   /* HLSQ_VS_CONFIG..GS_CONFIG（5 连续寄存器）+ HLSQ_CS_CONFIG */
   tu_cs_emit_pkt4(cs, REG_A5XX_HLSQ_VS_CONFIG, 5);
   for (unsigned i = 0; i < 5; i++) {
      tu_cs_emit(cs,
                 COND(st[i].v, A5XX_HLSQ_VS_CONFIG_ENABLED) |
                    A5XX_HLSQ_VS_CONFIG_CONSTOBJECTOFFSET(st[i].constoff) |
                    A5XX_HLSQ_VS_CONFIG_SHADEROBJOFFSET(st[i].instroff));
   }
   tu_cs_emit_pkt4(cs, REG_A5XX_HLSQ_CS_CONFIG, 1);
   tu_cs_emit(cs, 0);

   /* HLSQ_VS_CNTL..GS_CNTL: INSTRLEN */
   tu_cs_emit_pkt4(cs, REG_A5XX_HLSQ_VS_CNTL, 5);
   for (unsigned i = 0; i < 5; i++) {
      tu_cs_emit(cs,
                 A5XX_HLSQ_VS_CNTL_INSTRLEN(st[i].instrlen) |
                    COND(st[i].v && st[i].v->has_ssbo,
                         A5XX_HLSQ_VS_CNTL_SSBO_ENABLE));
   }

   /* SP_VS_CONFIG..GS_CONFIG（5 连续寄存器）+ SP_CS_CONFIG */
   tu_cs_emit_pkt4(cs, REG_A5XX_SP_VS_CONFIG, 5);
   for (unsigned i = 0; i < 5; i++) {
      tu_cs_emit(cs,
                 COND(st[i].v, A5XX_SP_VS_CONFIG_ENABLED) |
                    A5XX_SP_VS_CONFIG_CONSTOBJECTOFFSET(st[i].constoff) |
                    A5XX_SP_VS_CONFIG_SHADEROBJOFFSET(st[i].instroff));
   }
   tu_cs_emit_pkt4(cs, REG_A5XX_SP_CS_CONFIG, 1);
   tu_cs_emit(cs, 0);

   /* HLSQ_xS_CONSTLEN/INSTRLEN 每级一对 */
   static const uint16_t hlsq_constlen_regs[5] = {
      REG_A5XX_HLSQ_VS_CONSTLEN, REG_A5XX_HLSQ_FS_CONSTLEN,
      REG_A5XX_HLSQ_HS_CONSTLEN, REG_A5XX_HLSQ_DS_CONSTLEN,
      REG_A5XX_HLSQ_GS_CONSTLEN,
   };
   for (unsigned i = 0; i < 5; i++) {
      tu_cs_emit_pkt4(cs, hlsq_constlen_regs[i], 2);
      tu_cs_emit(cs, st[i].constlen);
      tu_cs_emit(cs, st[i].instrlen);
   }

   const struct ir3_shader_variant *fs = variants[MESA_SHADER_FRAGMENT];
   if (fs) {
      /* fd5_program_emit: HLSQ_CONTROL_0..4（FS 线程/face/ij/coord regids） */
      enum a3xx_threadsize fssz =
         fs->info.double_threadsize ? FOUR_QUADS : TWO_QUADS;

      uint32_t face_regid =
         ir3_find_sysval_regid(fs, SYSTEM_VALUE_FRONT_FACE);
      uint32_t samp_id_regid =
         ir3_find_sysval_regid(fs, SYSTEM_VALUE_SAMPLE_ID);
      uint32_t samp_mask_regid =
         ir3_find_sysval_regid(fs, SYSTEM_VALUE_SAMPLE_MASK_IN);
      uint32_t coord_regid =
         ir3_find_sysval_regid(fs, SYSTEM_VALUE_FRAG_COORD);
      uint32_t zwcoord_regid = coord_regid != regid(63, 0) ?
         coord_regid + 2 : regid(63, 0);
      uint32_t ij_regid[6];
      for (unsigned i = 0; i < ARRAY_SIZE(ij_regid); i++) {
         ij_regid[i] = ir3_find_sysval_regid(fs,
            (gl_system_value) (SYSTEM_VALUE_BARYCENTRIC_PERSP_PIXEL + i));
      }

      tu_cs_emit_pkt4(cs, REG_A5XX_HLSQ_CONTROL_0_REG, 5);
      tu_cs_emit(cs, A5XX_HLSQ_CONTROL_0_REG_FSTHREADSIZE(fssz) |
                        A5XX_HLSQ_CONTROL_0_REG_CSTHREADSIZE(TWO_QUADS) |
                        0x00000880); /* XXX fd5 */
      tu_cs_emit(cs, A5XX_HLSQ_CONTROL_1_REG_PRIMALLOCTHRESHOLD(63));
      tu_cs_emit(cs, A5XX_HLSQ_CONTROL_2_REG_FACEREGID(face_regid) |
                        A5XX_HLSQ_CONTROL_2_REG_SAMPLEID(samp_id_regid) |
                        A5XX_HLSQ_CONTROL_2_REG_SAMPLEMASK(samp_mask_regid) |
                        A5XX_HLSQ_CONTROL_2_REG_CENTERRHW(ij_regid[3]));
      tu_cs_emit(cs, A5XX_HLSQ_CONTROL_3_REG_IJ_PERSP_PIXEL(ij_regid[0]) |
                        A5XX_HLSQ_CONTROL_3_REG_IJ_LINEAR_PIXEL(ij_regid[3]) |
                        A5XX_HLSQ_CONTROL_3_REG_IJ_PERSP_CENTROID(ij_regid[1]) |
                        A5XX_HLSQ_CONTROL_3_REG_IJ_LINEAR_CENTROID(ij_regid[4]));
      tu_cs_emit(cs, A5XX_HLSQ_CONTROL_4_REG_XYCOORDREGID(coord_regid) |
                        A5XX_HLSQ_CONTROL_4_REG_ZWCOORDREGID(zwcoord_regid) |
                        A5XX_HLSQ_CONTROL_4_REG_IJ_PERSP_SAMPLE(ij_regid[2]) |
                        A5XX_HLSQ_CONTROL_4_REG_IJ_LINEAR_SAMPLE(ij_regid[5]));

      /* GRAS_CNTL / RB_RENDER_CONTROL0/1（fd5_program_emit） */
      uint32_t gras_cntl =
         COND(ij_regid[0] != regid(63, 0), A5XX_GRAS_CNTL_IJ_PERSP_PIXEL) |
         COND(ij_regid[1] != regid(63, 0), A5XX_GRAS_CNTL_IJ_PERSP_CENTROID) |
         COND(ij_regid[2] != regid(63, 0), A5XX_GRAS_CNTL_IJ_PERSP_SAMPLE) |
         COND(ij_regid[3] != regid(63, 0), A5XX_GRAS_CNTL_IJ_LINEAR_PIXEL) |
         COND(ij_regid[4] != regid(63, 0), A5XX_GRAS_CNTL_IJ_LINEAR_CENTROID) |
         COND(ij_regid[5] != regid(63, 0), A5XX_GRAS_CNTL_IJ_LINEAR_SAMPLE) |
         COND(fs->fragcoord_compmask != 0,
              A5XX_GRAS_CNTL_COORD_MASK(fs->fragcoord_compmask) |
                 A5XX_GRAS_CNTL_IJ_LINEAR_PIXEL) |
         COND(fs->frag_face, A5XX_GRAS_CNTL_IJ_LINEAR_PIXEL);
      tu_cs_emit_pkt4(cs, REG_A5XX_GRAS_CNTL, 1);
      tu_cs_emit(cs, gras_cntl);

      tu_cs_emit_pkt4(cs, REG_A5XX_RB_RENDER_CONTROL0, 2);
      tu_cs_emit(cs, gras_cntl); /* RB_RENDER_CONTROL0 位域与 GRAS_CNTL 相同 */
      tu_cs_emit(cs,
                 COND(samp_mask_regid != regid(63, 0),
                      A5XX_RB_RENDER_CONTROL1_SAMPLEMASK) |
                    COND(fs->frag_face, A5XX_RB_RENDER_CONTROL1_FACENESS) |
                    COND(samp_id_regid != regid(63, 0),
                         A5XX_RB_RENDER_CONTROL1_SAMPLEID));

      /* SP_FS_OUTPUT_REG(0..7): 颜色输出 regid */
      tu_cs_emit_pkt4(cs, REG_A5XX_SP_FS_OUTPUT_REG(0), 8);
      for (unsigned i = 0; i < 8; i++) {
         ...
         tu_cs_emit(cs, A5XX_SP_FS_OUTPUT_REG_REGID(color_regid) |
                           COND(color_regid & HALF_REG_ID,
                                A5XX_SP_FS_OUTPUT_REG_HALF_PRECISION));
      }

      /* fd5 固定值寄存器 */
      tu_cs_emit_pkt4(cs, REG_A5XX_VPC_GS_SIV_CNTL, 1);
      tu_cs_emit(cs, 0x0000ffff); /* XXX fd5 */
      tu_cs_emit_pkt4(cs, REG_A5XX_SP_SP_CNTL, 1);
      tu_cs_emit(cs, 0x00000010); /* XXX fd5 */
   }

   /* fd5_program_emit: HLSQ_UPDATE_CNTL（pipeline 级重发） */
   tu_cs_emit_pkt4(cs, REG_A5XX_HLSQ_UPDATE_CNTL, 1);
   tu_cs_emit(cs, 0x020fffff); /* XXX fd5 */

   /* fd5_program_emit 尾部: VFD_CONTROL_1..5（VS 系统值 regid） */
   {
      ...
      uint32_t instance_regid =
         ir3_find_sysval_regid(vs, SYSTEM_VALUE_INSTANCE_ID);
      tu_cs_emit_pkt4(cs, REG_A5XX_VFD_CONTROL_1, 5);
      tu_cs_emit(cs, A5XX_VFD_CONTROL_1_REGID4VTX(vertex_regid) |
                        A5XX_VFD_CONTROL_1_REGID4INST(instance_regid) |
                        0xfc0000);
      tu_cs_emit(cs, 0x0000fcfc); /* VFD_CONTROL_2 */
      tu_cs_emit(cs, 0x0000fcfc); /* VFD_CONTROL_3 */
      tu_cs_emit(cs, 0x000000fc); /* VFD_CONTROL_4 */
      tu_cs_emit(cs, 0x00000000); /* VFD_CONTROL_5 */
   }
...
{
   if (CHIP == A5XX) {
      tu5_emit_program_config(cs, prog, shaders, variants);
      return;
   }
                      ...
                      struct tu_shader **shaders)
{
   ...
   */
   ...
   tu_cs_begin_sub_stream(sub_cs, 512, &prog_cs);
   if (CHIP == A5XX)
      prog->vpc_stride = tu5_emit_vpc(&prog_cs, vs, fs);
   else
                      ...
                      const struct tu_cmd_buffer *cmd)
{
   ...
   if (CHIP == A5XX) {
      /* tu5xx: a5xx VFD_DECODE 无 OFFSET 字段，fetch 基址必须含属性偏移
       * （fd5_emit_vertex_bufs 模式），按属性槽逐个发射 FETCH+DECODE。 */
      for (uint32_t loc = 0; loc < attr_count; loc++) {
         ...
         if (vi->attributes_valid & (1u << loc)) {
            ...
            enum pipe_format pipe_format =
               vk_format_to_pipe_format(attr->format);
            const struct tu_native_format format = tu5_format_vtx(pipe_format);
            uint64_t base = cmd->state.vb[attr->binding].base + attr->offset;
            uint32_t vb_size = cmd->state.vb[attr->binding].size;

            tu_cs_emit_pkt4(cs, REG_A5XX_VFD_FETCH(loc), 4);
            tu_cs_emit_qw(cs, base);
            tu_cs_emit(cs, attr->offset < vb_size ? vb_size - attr->offset : 0);
            tu_cs_emit(cs, binding->stride);

            tu_cs_emit_pkt4(cs, REG_A5XX_VFD_DECODE(loc), 2);
            tu_cs_emit(cs,
                       A5XX_VFD_DECODE_INSTR_IDX(attr->binding) |
                       A5XX_VFD_DECODE_INSTR_FORMAT(
                          (enum a5xx_vtx_fmt) format.fmt) |
                       COND(binding->input_rate == VK_VERTEX_INPUT_RATE_INSTANCE,
                            A5XX_VFD_DECODE_INSTR_INSTANCED) |
                       A5XX_VFD_DECODE_INSTR_SWAP(format.swap) |
                       A5XX_VFD_DECODE_INSTR_UNK30 |
                       COND(!util_format_is_pure_integer(pipe_format),
                            A5XX_VFD_DECODE_INSTR_FLOAT));
            tu_cs_emit(cs, MAX2(1, binding->divisor));
         } else {
            tu_cs_emit_pkt4(cs, REG_A5XX_VFD_FETCH(loc), 4);
            for (unsigned i = 0; i < 4; i++)
               tu_cs_emit(cs, 0);
...
tu6_emit_vertex_stride(struct tu_cs *cs, const struct vk_vertex_input_state *vi)
{
   if (vi->bindings_valid) {
      ...
      for (unsigned i = 0; i < bindings_count; i++) {
         tu_cs_emit(cs, CHIP == A5XX ? REG_A5XX_VFD_FETCH_STRIDE(i)
                                     : REG_A6XX_VFD_FETCH_STRIDE(i));
...
{
   /* tu5xx: 无 GRAS_CL_Z_CLAMP / RB_Z_CLAMP 寄存器 */
   if (CHIP == A5XX)
      return 1 + vp->viewport_count * 6 + 1 + vp->viewport_count * 2 + 1;
...
tu6_emit_viewport(struct tu_cs *cs, const struct vk_viewport_state *vp)
{
   ...
   if (CHIP == A5XX) {
      /* tu5xx: 移植 fd5_emit_viewport，仅 viewport 0（GRAS_CL_VPORT_XOFFSET_0
       * 布局与 a6xx 相同：offset/scale 按 X/Y/Z 交错）。 */
      ...
      tu_cs_emit_pkt4(cs, REG_A5XX_GRAS_CL_VPORT_XOFFSET_0, 6);
      ...
      tu_cs_emit_pkt4(cs, REG_A5XX_GRAS_CL_GUARDBAND_CLIP_ADJ, 1);
      tu_cs_emit(cs,
                 A5XX_GRAS_CL_GUARDBAND_CLIP_ADJ_HORZ(guardband.width) |
                 A5XX_GRAS_CL_GUARDBAND_CLIP_ADJ_VERT(guardband.height));
      return;
   }
...
{
   if (CHIP == A5XX) {
      /* tu5xx: a5xx 只有 GRAS_SC_SCREEN_SCISSOR_TL_0/BR_0（fd5_emit.c），
       * 位排布与 a6xx 相同。 */
      assert(vp->scissor_count >= 1);
      ...
      tu_cs_emit(cs, A5XX_GRAS_SC_SCREEN_SCISSOR_TL_0_X(min_x) |
...
{
   /* tu5xx: GRAS_SU_POLY_OFFSET_SCALE/OFFSET/OFFSET_CLAMP 连续（0xe095..），
    * 浮点打包与 a6xx 相同。 */
   tu_cs_emit_pkt4(cs,
                   CHIP == A5XX ? REG_A5XX_GRAS_SU_POLY_OFFSET_SCALE
               ...
               uint32_t sample_mask)
{
   ...
   if (CHIP == A5XX) {
      /* tu5xx: 移植 fd5_blend_state_create。RB_MRT_CONTROL/BLEND_CONTROL
       * 位排布与 a6xx 相同，复用 A6XX 打包宏。 */
      for (unsigned i = 0; i < num_rts; i++) {
         uint32_t control = 0, blend_control = 0;
         if (i < cb->attachment_count) {
            const struct vk_color_blend_attachment_state *att = &cb->attachments[i];
            if (cb->color_write_enables & (1u << i)) {
               control = A6XX_RB_MRT_CONTROL(i,
                                             .rop_enable = cb->logic_op_enable,
                                             .rop_code = rop,
                                             .component_enable = att->write_mask).value;
               if (att->blend_enable) {
                  control |= A5XX_RB_MRT_CONTROL_BLEND |
                             A5XX_RB_MRT_CONTROL_BLEND2;
               }
               blend_control = A6XX_RB_MRT_BLEND_CONTROL(i,
                  .rgb_src_factor = tu6_blend_factor((VkBlendFactor)att->src_color_blend_factor),
                  .rgb_blend_opcode = tu6_blend_op(att->color_blend_op),
                  .rgb_dest_factor = tu6_blend_factor((VkBlendFactor)att->dst_color_blend_factor),
                  .alpha_src_factor = tu6_blend_factor((VkBlendFactor)att->src_alpha_blend_factor),
                  .alpha_blend_opcode = tu6_blend_op(att->alpha_blend_op),
                  .alpha_dest_factor = tu6_blend_factor((VkBlendFactor)att->dst_alpha_blend_factor)).value;
            }
         }

         tu_cs_emit_pkt4(cs, REG_A5XX_RB_MRT_CONTROL(i), 2);
         tu_cs_emit(cs, control);
         tu_cs_emit(cs, blend_control);
      }

      tu_cs_emit_pkt4(cs, REG_A5XX_SP_BLEND_CNTL, 1);
      tu_cs_emit(cs,
                 A5XX_SP_BLEND_CNTL_ENABLE_BLEND(blend_enable_mask) |
...
tu6_blend_constants_size(struct tu_device *dev,
                         const struct vk_color_blend_state *cb)
{
   /* tu5xx: F32 常量寄存器与 UINT/SINT 交错，需 4 次单寄存器写 */
   if (CHIP == A5XX)
      return 8;
...
{
   if (CHIP == A5XX) {
      /* tu5xx: RB_BLEND_RED/GREEN/BLUE/ALPHA_F32 交错排布（0xe1a1+2i） */
      static const uint32_t regs[4] = {
         REG_A5XX_RB_BLEND_RED_F32, REG_A5XX_RB_BLEND_GREEN_F32,
         REG_A5XX_RB_BLEND_BLUE_F32, REG_A5XX_RB_BLEND_ALPHA_F32,
      };
      for (unsigned i = 0; i < 4; i++) {
         tu_cs_emit_pkt4(cs, regs[i], 1);
         tu_cs_emit(cs, fui(cb->blend_constants[i]));
      }
      return;
   }
...
{
   /* tu5xx: SU_CNTL + CL_CNTL + PC_RASTER_CNTL + POINT_MINMAX/SIZE */
   if (CHIP == A5XX)
      return 9;
...
{
   if (CHIP == A5XX) {
      /* tu5xx: 移植 fd5_rasterizer_state_create + fd5_emit 的 rast 状态。 */

      tu_cs_emit_pkt4(cs, REG_A5XX_GRAS_SU_CNTL, 1);
      tu_cs_emit(cs,
                 A5XX_GRAS_SU_CNTL_LINEHALFWIDTH(rs->line.width / 2.0f) |
                 A5XX_GRAS_SU_CNTL_LINE_MODE(
                    rs->line.mode == VK_LINE_RASTERIZATION_MODE_BRESENHAM_EXT ?
                       BRESENHAM : RECTANGULAR) |
                 COND(rs->cull_mode & VK_CULL_MODE_FRONT_BIT,
                      A5XX_GRAS_SU_CNTL_CULL_FRONT) |
                 COND(rs->cull_mode & VK_CULL_MODE_BACK_BIT,
                      A5XX_GRAS_SU_CNTL_CULL_BACK) |
                 COND(rs->front_face == VK_FRONT_FACE_CLOCKWISE,
                      A5XX_GRAS_SU_CNTL_FRONT_CW) |
                 COND(rs->depth_bias.enable, A5XX_GRAS_SU_CNTL_POLY_OFFSET));

      tu_cs_emit_pkt4(cs, REG_A5XX_GRAS_CL_CNTL, 1);
      tu_cs_emit(cs,
                 COND(!vp->depth_clip_negative_one_to_one,
                      A5XX_GRAS_CL_CNTL_ZERO_GB_SCALE_Z));

      enum adreno_pa_su_sc_draw polygon_mode = tu5_polygon_mode(rs->polygon_mode);
      tu_cs_emit_pkt4(cs, REG_A5XX_PC_RASTER_CNTL, 1);
      tu_cs_emit(cs,
                 A5XX_PC_RASTER_CNTL_POLYMODE_FRONT_PTYPE(polygon_mode) |
                 A5XX_PC_RASTER_CNTL_POLYMODE_BACK_PTYPE(polygon_mode) |
                 COND(rs->polygon_mode != VK_POLYGON_MODE_FILL,
                      A5XX_PC_RASTER_CNTL_POLYMODE_ENABLE));

      tu_cs_emit_pkt4(cs, REG_A5XX_GRAS_SU_POINT_MINMAX, 2);
      tu_cs_emit(cs, A5XX_GRAS_SU_POINT_MINMAX_MIN(1.0f / 16.0f) |
                     A5XX_GRAS_SU_POINT_MINMAX_MAX(4092.0f));
      tu_cs_emit(cs, A5XX_GRAS_SU_POINT_SIZE(1.0f));
      return;
   }
...
{
   /* tu5xx: a5xx 模板状态寄存器（RB_STENCILREFMASK 等）布局不同，
    * 暂不支持深度/模板（首个三角形仅颜色附件）。 */
   if (CHIP == A5XX)
      return 0;
...
tu6_rb_depth_cntl_size(struct tu_device *dev,
                       ...
                       const struct vk_rasterization_state *rs)
{
   /* tu5xx: RB_DEPTH_CNTL 移植待后续深度支持，暂不发射 */
   if (CHIP == A5XX)
      return 0;
...
#define DRAW_STATE(name, id, ...) DRAW_STATE_COND(name, id, true, __VA_ARGS__)
   ...
   /* tu5xx: a5xx 的 VFD_FETCH 需要含属性偏移的 VB 基址（依赖 cmd 绑定的
    * vertex buffer），无法在管线创建期烘焙，保持为动态状态。 */
   if (CHIP != A5XX) {
      DRAW_STATE(vertex_input, TU_DYNAMIC_STATE_VERTEX_INPUT,
                 builder->graphics_state.vi, nullptr);
   } else {
      emit_pipeline_state(keep, remove, pipeline_set, tu_vertex_input_state,
                          ARRAY_SIZE(tu_vertex_input_state), true, builder);
   }
   ...
   struct tu_pipeline_builder *builder, struct tu_pipeline *pipeline)
{
   ...
   bool is_a5xx =
      builder->device->physical_device->info->chip == 5;
