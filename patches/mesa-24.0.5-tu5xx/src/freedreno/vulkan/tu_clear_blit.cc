                 ...
                 bool scissor)
{
   ...
   /* tu5xx: SP_2D_DST_FORMAT(CHIP) 为 a6xx 派发宏（a5xx 空 → 非法包头）。
    * a5xx 2D 引擎格式在 RB_2D_DST_INFO 内（fd5_blitter.c），此处跳过。 */
   if (CHIP != A5XX) {
           ...
           uint32_t rts_mask, bool z_scale, VkSampleCountFlagBits samples)
{
   ...
   /* tu5xx: HLSQ_INVALIDATE_CMD 为 a6xx 派发宏，a5xx 无对应 variant
    * （空对 → pkt4(reg=0) 非法包头）。a5xx 的 CP_LOAD_STATE4 无需
    * invalidate，跳过（fd5 亦从不写此寄存器）。 */
   if (CHIP != A5XX) {
   ...
   if (CHIP == A5XX) {
      /* tu5xx: 同上，a5xx 用 REG_A5XX_PC_RASTER_CNTL（0x12 = FRONT/BACK
       * PTYPE 均为 TRIANGLES，与 fd5_emit_restore 一致） */
   ...
   tu6_emit_msaa(cmd->device, cs, samples, false);
}
          ...
          VkSampleCountFlagBits samples)
{
   ...
   if (!(blit_param & R3D_DST_GMEM)) {
      ...
      /* tu5xx: RB_BIN_CONTROL(CHIP) 为 a6xx 派发宏（a5xx 空 → 非法包头）；
       * a5xx sysmem 无 binning，跳过 */
      if (CHIP != A5XX)
...
template <>
VKAPI_ATTR void VKAPI_CALL
tu_CmdFillBuffer<A5XX>(VkCommandBuffer commandBuffer,
                       VkBuffer dstBuffer,
                       VkDeviceSize dstOffset,
                       VkDeviceSize fillSize,
                       uint32_t data)
{
   /* tu5xx: A506 固件不支持 CP_MEM_WRITE(0x3d)（fd5 亦从不使用，实测
    * 包头被固件忽略后填充值被当作命令解析）。模板数据也不能内嵌命令流：
    * CP 顺序解析会把流内裸数据当指令（opcode error 0xAABBCCDD 根因）。
    * 改用内核先例（a5xx_gpu.c 用 pkt4 写 CP_SCRATCH）+ fd5 验证过的
    * CP_REG_TO_MEM(0x3e)：pkt4 写 CP_SCRATCH_REG0/1=(data,data)，
    * REG_TO_MEM CNT=2 一次自举 8 字节到 dst 头部，随后 CP_MEM_TO_MEM(0x73)
    * DOUBLE 模式从 dst 头部顺序拷贝（每包带 WAIT_FOR_MEM_WRITES 保序）。 */
   TU_FROM_HANDLE(tu_cmd_buffer, cmd, commandBuffer);
   TU_FROM_HANDLE(tu_buffer, buffer, dstBuffer);
   struct tu_cs *cs = &cmd->cs;
   ...
   uint64_t dst_va = buffer->iova + dstOffset;
   uint32_t dwords = fillSize / 4;
   if (!dwords)
      return;

   tu_cs_emit_pkt4(cs, REG_A5XX_CP_SCRATCH_REG(0), 2);
   tu_cs_emit(cs, data);
   tu_cs_emit(cs, data);

   tu_cs_emit_wfi(cs);

   /* 自举：scratch 寄存器 -> dst 头部（最多 8 字节） */
   uint32_t first = MIN2(dwords, 2);
   tu_cs_emit_pkt7(cs, CP_REG_TO_MEM, 3);
   tu_cs_emit(cs, CP_REG_TO_MEM_0_REG(REG_A5XX_CP_SCRATCH_REG(0)) |
                  CP_REG_TO_MEM_0_CNT(first));
   tu_cs_emit_qw(cs, dst_va);
   dst_va += first * 4;
   dwords -= first;

   /* 剩余部分从 dst 头部 8 字节模板继续拷贝 */
   uint64_t src_va = buffer->iova + dstOffset;
   while (dwords >= 2) {
      tu_cs_emit_pkt7(cs, CP_MEM_TO_MEM, 5);
      tu_cs_emit(cs, CP_MEM_TO_MEM_0_DOUBLE |
                     CP_MEM_TO_MEM_0_WAIT_FOR_MEM_WRITES);
      tu_cs_emit_qw(cs, dst_va);
      tu_cs_emit_qw(cs, src_va);
      dst_va += 8;
      dwords -= 2;
   }
   if (dwords) {
                            ...
                            const VkClearRect *rects)
{
    ...
    */
   /* tu5xx: 无 CP_SET_DRAW_STATE 组表，无需逐组 DISABLE；
    * dirty 标记确保后续 draw 全量重发状态（见 tu_cs_emit_draw_state） */
   if (CHIP != A5XX) {
