...
class Parser(object):
	...
	def dump_reg_variants(self, regname, variants):
		...
		for variant in variants.keys():
			...
			print("  } else")
		# Regs only defined for other gens: emit nothing (reg==0 pairs are
		# skipped by tu_cs_emit_regs), instead of falling off the end.
		print("    { assert(!\"invalid variant\"); return (struct fd_reg_pair) {}; }")
		print("}")
